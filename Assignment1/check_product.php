  ...
  <div class="top_right">
    ...
    <table class="check_result">
      ...
      if (array_key_exists("id", $_GET)) {
        ...
        if ($num_row) {
          echo '<tr><td>Product Name</td><td>Quantity</td><td>Price</td><td>Stock</td><tr>';
          while ($a_row = mysqli_fetch_assoc($result)) {
            $name =  $a_row['product_name'];
            $unit_quantity =  $a_row['unit_quantity'];
            $price = $a_row['unit_price'];
            $stock = $a_row['in_stock'];
            echo '<td>' . $name . '</td>';
            echo '<td>' . $unit_quantity . '</td>';
            echo '<td>' . $price . '</td>';
            echo '<td>' . $stock . '</td>';
            echo '<td><input type="button" value="add" class="button" ';
            echo "onclick = ";
            echo '"window.location.href=';
            echo "'change_cart.php?id=$id&name=$name&price=$price&unit_quantity=$unit_quantity";
            echo "'";
            echo '")></td></tr>';
          }
        } else {
      ...
      if (array_key_exists("search_box", $_GET)) {
        //receive product id
        $name = $_GET['search_box'];
        $link = mysqli_connect("localhost", "potiro", "pcXZb(kL", "poti");
        $query_string = "select * from products where product_name = '$name'";
        $result = mysqli_query($link, $query_string);
        $num_row = mysqli_num_rows($result);
